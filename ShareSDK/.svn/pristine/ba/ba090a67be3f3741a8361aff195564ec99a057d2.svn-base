 ...
 * 5，分享视频
 * ============================================================
 *      在重构之前该类的耦合还是蛮严重的。无法扩展，
 *      只能用这里面写好的微信，QQ，Sina三种。
 *      现在有时间了，回过头把代码重构了下，利用接口+装饰者模式，
 *      来尽量解耦合。
 * ============================================================
 */
public class CommonShareOperate extends Object implements IShareOperate{
     ...
     * @param shareOperate
     */
    public CommonShareOperate(Context context,IShareOperate shareOperate){
    ...
    public void shareText(CommonShareTextOnly shareTextBean, Object object) {
        ...
        if(mShareOperate != null){
            mShareOperate.shareText(shareTextBean, object);
            return;
        }

        //当上层没有写装饰者时，就使用默认的微信，QQ,Sina三种。
    ...
    public void sharePic(CommonShareBaseBean sharePicBean, Object iPlatform) {
        sharePicOnly(sharePicBean, iPlatform);
    }
    ...
    public void sharePicOnly(CommonShareBaseBean sharePicBean, Object object) {
        if(mShareOperate != null){
            mShareOperate.sharePic(sharePicBean, object);
            return;
        }
