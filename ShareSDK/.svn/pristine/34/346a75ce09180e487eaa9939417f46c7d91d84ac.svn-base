...
public class CircleFlowIndicator extends View implements FlowIndicator,
    ...
    private float circleSeparation = 5*radius+radius;
    ...
    public CircleFlowIndicator(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.CommonShareCircleFlowIndicator);

        // Gets the inactive circle type, defaulting to "fill"
        int activeType = a.getInt(R.styleable.CommonShareCircleFlowIndicator_activeType,
                STYLE_FILL);
        ...
        int activeColor = a
                .getColor(R.styleable.CommonShareCircleFlowIndicator_activeColor,
                        activeDefaultColor);

        // Gets the inactive circle type, defaulting to "stroke"
        int inactiveType = a.getInt(
                R.styleable.CommonShareCircleFlowIndicator_inactiveType, STYLE_STROKE);

        int inactiveDefaultColor = 0x44FFFFFF;
        // Get a custom inactive color if there is one
        int inactiveColor = a.getColor(
                R.styleable.CommonShareCircleFlowIndicator_inactiveColor,
                inactiveDefaultColor);

        // Retrieve the radius
        radius = a.getDimension(R.styleable.CommonShareCircleFlowIndicator_flowradius, 6.0f);

        circleSeparation = a.getDimension(R.styleable.CommonShareCircleFlowIndicator_circleSeparation, 4*radius+radius);
        activeRadius = a.getDimension(R.styleable.CommonShareCircleFlowIndicator_activeRadius, 0.5f);
        // Retrieve the fade out time
        fadeOutTime = a.getInt(R.styleable.CommonShareCircleFlowIndicator_fadeOut, 0);

        mCentered = a.getBoolean(R.styleable.CommonShareCircleFlowIndicator_flowcentered, false);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (viewFlow != null) {
            ...
            Log.e("flowIndicator","count="+count);
        }
